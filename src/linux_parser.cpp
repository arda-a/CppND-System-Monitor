...
int LinuxParser::TotalProcesses() {
  ...
  if (stream.is_open()) {
    while (std::getline(stream, line)) {
      std::istringstream linestream(line);
      linestream >> processes >> strProcessCount;
      if (processes == "processes") {
        processCount = std::stoi(strProcessCount);
        break;
      }
    }
  }
...
int LinuxParser::RunningProcesses() {
  string strProcessCount;
  string procs_running, line;
  int processCount = 0;
  std::ifstream stream(kProcDirectory + kStatFilename);
  if (stream.is_open()) {
    while (std::getline(stream, line)) {
      std::istringstream linestream(line);
      linestream >> procs_running >> strProcessCount;
...
string LinuxParser::User(int pid) {
  ...
  if (passwdStream.is_open()) {
    while (std::getline(passwdStream, line)) {
      std::replace(line.begin(), line.end(), ':', ' ');
      std::istringstream linePassStream(line);
      while (linePassStream >> key >> value >> value2)
        if (value2.compare(Uid(pid)) == 0) return key;
    }
  }
